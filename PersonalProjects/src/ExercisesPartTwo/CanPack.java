...
public class CanPack {
    ...
    public static boolean canPack(int bigCount, int smallCount, int goal) {
        ...
        firstHalf = (bigBag * (bigCount/2)) + (smallBag * (smallCount/2));
        int remainingBigCount = bigCount - (bigCount/2);
        int remainingSmallCount = smallCount - (smallCount/2);
        secondHalf = (remainingBigCount*5) + remainingSmallCount;
        int totalPackage = firstHalf + secondHalf;
        ...
        else if(totalPackage == goal) {
            return true;
        }
        else if(totalPackage > goal) {
            if((firstHalf + 1) == secondHalf || (firstHalf - 1) == secondHalf ||
                    (secondHalf - 1 ) == firstHalf || (secondHalf+1) == firstHalf || firstHalf == secondHalf){
                return true;
            }
            else {
                return false;
